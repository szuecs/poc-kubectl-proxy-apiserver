...
================
Example:
  %% %s -u admin -p password -f front.end.com -b back.end.com
`, bin, bin)
		flag.PrintDefaults()
	}
	flag.StringVar(&username, "username", "", "Username to authenticate with BasicAuth to the Backend")
	flag.StringVar(&password, "password", "", "Password to authenticate with BasicAuth to the Backend")
	flag.StringVar(&frontend, "frontend", "http://127.0.0.1:7070", "Frontend url of this application")
	flag.StringVar(&backend, "backend", "", "Backend url is the target url (of the kube-apiserver) to proxy to")
}
func main() {
	flag.Parse()
